...
```{r}
love.plot(match_Low_Dim, binary = "std",drop.distance = TRUE, 
...
```{r}
#plot(summary(match_High_Dim, subclass = TRUE),
     #var.order = "unmatched", abs = FALSE)

#plot(summary(match_High_Dim, subclass = TRUE),
     #var.order = "unmatched", abs = TRUE)
#axis(side=1, tick=FALSE,lwd= 0)
#set.cobalt.options(binary = "std")

list1 <- seq(1, 200, by=1)
empty_vec <- seq(1, 200, by=1)

#empty_vec <- rep(" ", 200)



empty_vec=paste0(empty_vec," ")
...
fit1 <- lm(Y ~ A, data = g.matches_high, weights = weights)
result_high_ATE<-coeftest(fit1, vcov. = vcovCL, cluster = ~subclass)["A",,drop=FALSE]
ATE_fullmatching_high <- result_high_ATE[1][1]
```
...
result_low_ATE<-coeftest(fit2, vcov. = vcovCL, cluster = ~subclass)[1:2,]
ATE_fullmatching_low <- result_low_ATE[2][1]
```
